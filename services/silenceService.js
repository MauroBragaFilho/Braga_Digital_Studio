    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('close', code => {
        ...
        resolve({
          ...
          isVideo: !!video,
          codec: video ? video.codec_name : (audio ? audio.codec_name : 'unknown'),
          width: video?.width || 0,
  ...
  async processQueue(config) {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const args = ['-y', '-i', filePath, '-filter_complex', filterComplex];
        
        if (info.isVideo) {
            args.push('-map', '[vfinal]', '-map', '[afinal]');
            // Fast software encoding to preserve exact trims without HW encoder bugs on concat
