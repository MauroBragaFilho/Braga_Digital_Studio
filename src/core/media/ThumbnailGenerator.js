...
class ThumbnailGenerator {
    ...
    async generate(videoPath, uuid, duration = 0) {
        ...
        if (isPhoto) {
            command = `"${this.ffmpegPath}" -i "${thumbSource}" -vf "scale=320:-1" -vframes 1 -q:v 2 -y "${outputPath}"`;
        } else {
            command = `"${this.ffmpegPath}" -ss ${timeString} -i "${thumbSource}" -vframes 1 -q:v 2 -y "${outputPath}"`;
        }
